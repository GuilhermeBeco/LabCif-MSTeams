...
def criarObjetosDeCriacaoDeEquipas():
    ...
    while line := logTeamsCreation.readline():
        ...
        if "isSipDisabled" in line:
            ...
            # verificar se contacto ja existe no array de contactos
            # se nao existir, entao adiciona
            for ac in list(arrayContactos):
                # print(orgid != ac)
                if orgid != ac:
                    contacto = Contacto(username, email, orgid)
                    arrayContactos[orgid] = contacto
            # ir buscar contacto do criador da conversacao
            creator = initiatororgid
            if initiatororgid in arrayContactos:
                creator = arrayContactos[creator]
                creator = creator.toString()
            # ir buscar contacto do(s) membro(s) a ser(em) adicionado(s)
            members = []

            for member_orgid in targets:
                if member_orgid in arrayContactos:
                    members.append(arrayContactos.get(member_orgid).toString())
                else:
                    members.append(member_orgid)

            # criar objeto de ConversationCreationDetails()
            conversation_details = ConversationCreationDetails(conversationid, date, creator, members)
            logObjetoscriacaoEquipas.write(conversation_details.toString() + "\n")
    logObjetoscriacaoEquipas.close()
    logTeamsCreation.close()
